...
class SplideListBuilder extends SplideListBuilderBase {
  ...
  public function render() {
    ...
    $build['description'] = [
      '#markup' => $this->t("<p>Manage the Splide optionsets. Optionsets are Config Entities.</p><p>By default, when this module is enabled, a single optionset is created from configuration. Install Splide X module to speed up by cloning them. Use the Operations column to edit, clone and delete optionsets.<br /><strong>Important!</strong> Avoid overriding Default optionset as it is meant for Default -- checking and cleaning. Use Duplicate instead. Otherwise messes are yours.<br />Splide doesn't need Splide UI to run. It is always safe to uninstall Splide UI once done with optionsets.</p>"),
    ];
